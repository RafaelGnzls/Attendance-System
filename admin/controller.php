...
if(isset($_POST['Sign_in']))
{
  ...
  if (!$row = $result->fetch_assoc()){
    echo '<script>
      ...
      if ($row['id'] === '1')
            header("Location: employee_attendance_GM.php");
      if ($row['id'] === '2')
            header("Location: employee_attendance_AUB.php");
      if ($row['id'] === '3')
            header("Location: employee_attendance_CLTS.php");
      if ($row['id'] === '4')
            header("Location: employee_attendance_HC.php");
      if ($row['id'] === '5')
            header("Location: employee_attendance_BMI.php");  
      if ($row['id'] === '6' || '7')
...
if(isset($_POST['add_employee']))
{
  ...
  $query = "INSERT INTO emp_list (emp_card, emp_fname, emp_lname, emp_position, emp_address, emp_contact, emp_gender, emp_timein, emp_timeout, sched_id, emp_regdate, emp_photo, campaign_id)
                          VALUES ('$tag', '$fname', '$lname', '$position', '$address', '$contact', '$gender', '$in', '$out', '$sched', '$regdate', '$target_file', '$refcamp')";
    ...
    while($row = mysqli_fetch_array($result))
    {
         ...
         $output .= '
              ...
              <div class="form-group row">
                ...
                <div class="col-sm-7">
                  <select name="update_campaign" class="form-control" value="'.$refcamp.'">
                    <option hidden> - Select -</option>
                     <option value="1">HOMECREDIT</option>
                     <option value="2">AUB</option>
...
if(isset($_POST["emp_update"]))
 {
   ...
   $refcamp = $_POST['update_campaign'];

   $sql = "UPDATE emp_list SET emp_fname = '".$fname."', emp_lname = '".$lname."', emp_address = '".$address."', emp_contact = '".$contact."', emp_gender = '".$gender."', campaign_id = '".$refcamp."'
   WHERE emp_id = '".$card."'";
   $result = mysqli_query($db, $sql);
